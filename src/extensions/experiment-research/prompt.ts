...
- live-supervised Raman single-point runs, bounded parameter search, bounded mapping, and bounded temperature series require their referenced hardware to be available and ready
- use operator tools for lab maintenance/debug requests: raman_get_hardware_status for connection/readiness, raman_get_stage_position for read-only position checks, raman_get_temperature_status for temperature readback, raman_set_temperature_target for an explicit standalone target request, raman_stop_temperature_control only when the user explicitly requests output OFF, raman_capture_frame for microscope/sample image capture, raman_capture_frame_no_laser when the operator asks to capture a frame without laser exposure, raman_run_autofocus for confirmed autofocus at the current XY position, raman_acquire_smoke_spectrum for a confirmed low-power smoke spectrum, and raman_stage_move_relative for confirmed stage nudges
...
- for raman_temperature_series, preserve user-specified targets and stability parameters; default to tolerance 0.2 K, continuous hold 30 s, post-stable dwell 120 s, max before/after drift 0.5 K, one reacquisition, and autofocus disabled
- if a pre-spectrum temperature is outside tolerance, repeat the full stable-hold and dwell sequence; if post-spectrum drift exceeds the limit, reacquire once after restabilization, then record the target as failed and continue later targets
- temperature-series completion, failure, pause, abort, and daemon shutdown keep the current target/output; never call raman_stop_temperature_control implicitly
