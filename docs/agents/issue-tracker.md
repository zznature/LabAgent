...
External PRs do not run through the issue triage queue. Treat collaborator PRs as code review work, not incoming request tickets.
...
- **Frontier query**: list the map's open children (`gh issue list --state open`, scoped to the map's sub-issues / task list), drop any with an open blocker or assignee; first in map order wins.
