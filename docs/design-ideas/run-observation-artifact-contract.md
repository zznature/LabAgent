...
MVP 已实现 Run Records Module、atomic descriptor/index、ordered events、heartbeat/error snapshot projection、
fixed artifact hierarchy、四种 Raman canonical profiles（包括 autofocus 的两张代表帧）、
same-run retry/resume、read adapter 与 operator operation scope。
...
**当前状态（部分完成）**：canonical frame、spectrum、autofocus、evaluation profiles 已接入；
autofocus 已关联 pre-focus / accepted-focus canonical frames，并对缺失 representation fail closed。
ProcedureSpec 的显式 `capture_frame` 已支持 `pre_focus` / `post_focus` / `observation` role 与
laser-off capture 语义；同一 attempt 的多次 frame capture 使用 action-scoped identity，避免后一次
覆盖或复用前一次 canonical descriptor。
Python adapter 的 typed source candidates 与上述 legacy helper 替换仍待完成，因此 Phase C 尚不能标记完成。
...
3. **Pause/resume**：runId 不变，重新执行产生新 attempt，旧证据不覆盖。
   最后一个 unit 执行期间收到 abort 时，在接受 attempt 前进入 `aborted`，不得错误完成 run。
4. **Canonical failure**：source spectrum 保留，但缺 scientific units 时 canonical artifact failed 且前端不可绘制。
   terminal RunState 同时保留 failed unit、typed error 与 publication failure 前已经产生的 artifact refs。
