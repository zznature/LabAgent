...
- `data` failure 表示硬件动作已经返回，但原始实验文件未能被正式保存或归档。
...
`source_artifact_unavailable` 必须来自 driver/runtime 对文件保存事实的结构化判断，不能从任意异常 message
猜测。该错误在 linear、parameter search 与 mapping scheduler 中按 data retry policy 创建一次 immediate
immutable retry attempt；data failure 不进入 final retry queue。重试耗尽后当前 point 失败，但 scheduler
继续后续 point，且 data failure 不计入 `maxConsecutiveFailures`，也不受 `stopOnError` 影响。
...
这是 kernel 判断 pause / retry / abort 的基础。Artifact publication lifecycle 与 run lifecycle 是两条
独立状态线：Source Artifact publication 失败会形成 point-level data failure；Canonical Artifact 因语义、
格式或 representation 问题失败时，只降低结构化分析能力，不把已完成的硬件动作改写成 run failure。
若派生 evaluation metrics 缺失，runtime 发布 diagnostic artifact、跳过该次结构化 evaluation，并继续 run。

run 因连续失败阈值结束时，顶层错误同时保留 terminal error 和
`payload.triggeringError`，避免聚合错误覆盖最初的设备或契约根因。

对 linear / repeated-single-point 执行，`stopOnError` 的边界也必须由 error scope 决定：

- `stopOnError = false` 且错误 scope 为 `action` 或 `unit` 时，当前 attempt 记录为 failed，
  当前 logical unit 记录最终失败，然后继续下一个 unit；
- error scope 为 `run` 时，无论 `stopOnError` 如何都必须结束 run；
- terminal failure 必须把当前 attempt 已产生的 artifact refs、最终 `failedUnits` 和 typed
  `errorState` 同步到 RunState 与 Run Records，不能只留下事件或 source 文件。

这样 `stopOnError = false` 表达的是“允许独立实验单元失败后继续”，而不是吞掉 run-level
契约、控制或系统性硬件失败。Artifact publication 自身仍 fail closed 并保留诊断 descriptor，
但不再通过异常隐式决定 run lifecycle。
