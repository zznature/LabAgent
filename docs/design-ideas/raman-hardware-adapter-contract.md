...
其中 `raman_capture_laser_off_frame` 属于 operator 的无激光现场观察能力：
它应通过 runtime 的 `frame.capture_laser_off` 复合 action 要求 LabSpec worker 先关闭/验证 laser，
再返回 frame artifact/path；它不应走 Raman 采谱 action，也不应假设普通 frame capture 已经关闭 laser。
