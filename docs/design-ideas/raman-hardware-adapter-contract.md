...
2. `frame_provider`
3. `spectrometer`
4. `temperature_controller`（可选；仅变温 procedure 需要）

如果未来把 camera、laser interlock 单独抽出，也应继续沿用同样模式。
...
### 4.4 示例：temperature controller 资源
...
串口由 lab/local config 决定，不进入 `ProcedureSpec`。`operatingRange` 表达已安装设备的能力边界，用于拒绝不支持的 target/ramp；它不是把升温本身归类为 safety risk。
...
stage、temperature controller 与 LabSpec frame 会话只在首次使用时连接、整个 run 复用；
...
- **空闲释放**：`daemon.idleShutdownMs`（默认 30000ms）无请求后 daemon 干净退出并释放串口，
  下个 action 再次 spawn。
- **温控保持**：daemon 退出只 disconnect serial session，不调用 controller `stop()`；run completed、failed、paused、aborted 也同样保持当前 target/output。只有显式 `temperature.stop` 将 output range 设置为 OFF。
...
`configure_target` 固定使用配置里的 automatic control mode、loop channel、output range 与 bounded ramp；不向 Agent 暴露 manual heater power。`disconnect` 与 `stop` 的语义必须分离。
...
其中温度 operator tools 的边界是：状态读取可直接执行；目标设置只响应用户明确的 standalone target 请求；正式“稳定后采谱”使用 `raman_temperature_series`；只有用户明确要求 stop/OFF 时调用 `raman_stop_temperature_control`。
...
```text
temperature.read_snapshot
temperature.configure_target
temperature.stop
```

`temperature.stop` 不是 run cleanup action，不能由完成、失败、暂停、abort 或 daemon shutdown 隐式触发。
...
变温采集使用独立 `raman_temperature_series` procedure：`plan.targetsK` 保存有序目标；`domain.temperature.stability` 保存 tolerance / continuous hold / dwell / poll / timeout；`domain.temperature.driftPolicy` 保存 before/after 最大漂移、最多一次重采与继续后续温度的规则。温度 target 不散落在 semantic step 参数或 driver command 中。
...
- temperature controller 配置存在且 target/ramp 处于 operating range；实时连接在第一次 temperature action 时惰性建立
...
- 每次 spectrum attempt 对应的 temperature evidence JSON（target、attempt、before/after snapshots、drift 与判定）
...
- `temperature_connection_error`
- `temperature_snapshot_failed`
- `temperature_stability_timeout`
...
除领域错误外，daemon 传输层（§4.6）还会归一化以下进程级错误码，同样带三个布尔标志：
...
1. 固定 stage / frame provider / spectrometer / optional temperature controller 资源。
...
5. 用 temperature series 校验同一分层能否承载跨设备实验维度，再平移到电化学或别的仪器。
