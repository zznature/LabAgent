...
$DriverSrc = Join-Path $WorkspaceRoot "lab-config/drivers/raman-python"
$DriverRoot = Join-Path $LabAgentsRepo "src/drivers"
$DriverDst = Join-Path $DriverRoot "raman-python"

if (-not (Test-Path $DriverSrc)) {
	throw "Missing workspace Raman Python driver source: $DriverSrc"
}

if (-not (Test-Path $DriverRoot)) {
	throw "Missing repository driver root: $DriverRoot"
}
...
if ($ResolvedDriverDstParent -ne $ResolvedDriverRoot) {
	throw "Refusing to sync driver outside repository driver root: $DriverDst"
}
...
Write-Output "Raman Python driver reverse synced: $DriverDst"
